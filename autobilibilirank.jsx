// @include 'json2.js';
var YUME = 1277009809
var WEEK_NUM = Math.round((Date.now() / 1000 - YUME) / 3600 / 24 / 7)

var CompFPS = 60
var CompSize = [1920, 1080]
var RankSize = [1440, 810]
var Prefix = './ranking/list1/'
var Regex =
    /- :rank: (\d+)\n {2}:name: (\w+)\n {2}:length: (\d+)\n {2}:offset: (\d+)(\n {2}:short: \d+)?(\n {2}:no_pause: true)?/gm
var Subst = '$1: ["$2", $3, $4],'
var Parts = [3, 5, 7, 9, 11, 13, 15, 16]
var file = new File('LostFile.json')
file.open('r')
var content = file.read()
file.close()
var LostVideos = JSON.parse(content)['name']
var RankDataList = []
for (var n = 0; n < Parts.length; n++) {
    var file = new File(Prefix + WEEK_NUM + '_' + Parts[n] + '.yml')
    file.open('r')
    var content = file.read()
    file.close()
    var RankList = content.replace(Regex, Subst).replace('\'', '"').replace('---', '{') + '}'
...
var FinalComp = app.project.items.addComp('周刊哔哩哔哩排行榜#' + WEEK_NUM, CompSize[0], CompSize[1], 1, 1, CompFPS)
var Part_1 = app.project.items.addComp('Part_1', CompSize[0], CompSize[1], 1, 40.5, CompFPS)
var Part_2 = app.project.items.addComp('Part_2', CompSize[0], CompSize[1], 1, 4, CompFPS)
var Part_3 = app.project.items.addComp('Part_3', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_4 = app.project.items.addComp('Part_4', CompSize[0], CompSize[1], 1, 4, CompFPS)
var Part_5 = app.project.items.addComp('Part_5', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_6 = app.project.items.addComp('Part_6', CompSize[0], CompSize[1], 1, 49.1, CompFPS)
var Part_7 = app.project.items.addComp('Part_7', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_8 = app.project.items.addComp('Part_8', CompSize[0], CompSize[1], 1, 4, CompFPS)
var Part_9 = app.project.items.addComp('Part_9', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_10 = app.project.items.addComp('Part_10', CompSize[0], CompSize[1], 1, 33, CompFPS)
var Part_11 = app.project.items.addComp('Part_11', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_12 = app.project.items.addComp('Part_12', CompSize[0], CompSize[1], 1, 4, CompFPS)
var Part_13 = app.project.items.addComp('Part_13', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_14 = app.project.items.addComp('Part_14', CompSize[0], CompSize[1], 1, 14.4, CompFPS)
var Part_15 = app.project.items.addComp('Part_15', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_16 = app.project.items.addComp('Part_16', CompSize[0], CompSize[1], 1, 60, CompFPS)
var Part_17 = app.project.items.addComp('Part_17', CompSize[0], CompSize[1], 1, 25, CompFPS)
var Part_18 = app.project.items.addComp('Part_18', CompSize[0], CompSize[1], 1, 60, CompFPS)

var StaticFolder = app.project.items.addFolder('StaticFootage')
var WeeklyFolder = app.project.items.addFolder('WeeklyFootage')
...
for (var key in StaticFootage) {
    var FootageFile = new ImportOptions(File(StaticFootage[key]))
    FootageFile.ImportAs = ImportAsType.FOOTAGE
    var FileItem = app.project.importFile(FootageFile)
    FileItem.name = key
    FileItem.parentFolder = StaticFolder
}

for (var n = 0; n < RankDataList.length; n++) {
    // IMPORT VIDEO
    for (var key in RankDataList[n]) {
        var FileBaseName = RankDataList[n][key][0]
        var FileFullPath = Prefix + FileBaseName + '.mp4'
...
// ITEM INDEX
var ResourceID = {}
for (var n = 1; n <= app.project.items.length; n++) {
    ResourceID[app.project.items[n].name] = n
}

// FUNCTION
function AddLayer(Target, Name, Duration, Offset) {
    var NewLayer = Target.layers.add(app.project.items[ResourceID[Name]], Duration)
    NewLayer.startTime = Offset
    return NewLayer
}

function AddAudioProperty(Target, Ptype, Duration, Offset, Direction) {
    var NewProperty = Target.property('Audio Levels')
    if (Ptype == 1) {
        // 1/4 circle
        if (Direction == 1) {
            // fade in
            for (var t = Offset; t <= Offset + Duration; t += Duration / CompFPS) {
...
function AddVideoProperty(Target, Ptype, Duration, Offset, Direction) {
    if (Ptype == 1) {
        // Opacity
        var NewProperty = Target.property('Opacity')
        if (Direction == 1) {
            // fade in
            for (var t = Offset; t <= Offset + Duration; t += Duration / CompFPS) {
    ...
    if (Ptype == 2) {
        var NewProperty = Target.property('Effects').addProperty('ADBE Linear Wipe')
        if (Direction == 1) {
            // fade in
...
function AddRankPart(Target, RankData, FirstRank, AddNEXT, AddProperty, Offset, IsTop) {
    var SortRank = []
    var InvalidPos = [1100, 777]
    var PartSize = RankSize
    for (var key in RankData) {
        SortRank[SortRank.length] = key
    }
    var LastRank = Math.max.apply(Math, SortRank)
    FirstRank = Math.min.apply(Math, SortRank)
    for (var i = 0; LastRank - i >= FirstRank; i++) {
        if (!(LastRank - i in RankData)) {
            continue
        }
        var VideoFile = RankData[LastRank - i][0]
        var VideoMask = RankData[LastRank - i][0] + '_'
        var TopImage = RankData[LastRank - i][0] + '_m'
        var VideoDuration = RankData[LastRank - i][1]
        var TrueDuration = app.project.items[ResourceID[VideoFile]].duration
        if (TrueDuration < VideoDuration) {
            VideoDuration = TrueDuration
        }
        var VideoOffset = RankData[LastRank - i][2]
        VideoOffset = 0
        var NewVideoLayer = AddLayer(Target, VideoFile, VideoDuration, Offset - VideoOffset)
        ...
        if (IsTop) {
            ...
            PartSize = CompSize
            var LogoLayer = AddLayer(Target, '周刊Logo', VideoDuration + 0.6, Offset)
        ...
        var OrigSize = NewVideoLayer.sourceRectAtTime(NewVideoLayer.inPoint, false)
        ...
        if (IsInvalid == true) {
            VideoOffset = 0
            var InvalidLayer = AddLayer(Target, 'NotFound', VideoDuration, Offset)
            ...
            var InvalidText = AddLayer(Target, '视频失效', VideoDuration, Offset)
            InvalidText.inPoint = Offset
            InvalidText.outPoint = Offset + VideoDuration
            InvalidText.property('Position').setValue(InvalidPos)
        ...
        if (IsTop) {
            var BlackLayer = Target.layers.addSolid([0, 0, 0], '黑底', CompSize[0], CompSize[1], 1, 12)
            BlackLayer.inPoint = Offset - VideoOffset
            var NewVideoLayerS = AddLayer(Target, VideoFile, 12, Offset - VideoOffset)
            ...
            if (IsInvalid == true) {
                VideoOffset = 0
                var LostVideoLayerS = AddLayer(Target, 'NotFound', VideoDuration, Offset)
                ...
                //     'text.sourceText.createStyle().setFont("FZY4K--GBK1-0").setFillColor(hexToRgb("CC0000")).setFontSize(48);';
                var InvalidTextS = AddLayer(Target, '视频失效', VideoDuration, Offset)
            ...
            var TopRankLayer = AddLayer(Target, TopImage, 5, Offset)
            AddVideoProperty(TopRankLayer, 1, 1, Offset, 1)
            Offset = Offset + VideoDuration + 0.6
        } else {
            var NewVideoLayer_mask = AddLayer(Target, VideoMask, VideoDuration, Offset)
            if (AddNEXT && LastRank - i > FirstRank) {
                var NextBGMLayer = AddLayer(Target, 'NEXT_BGM', 1, Offset + VideoDuration)
                var NextLayer = AddLayer(Target, 'NEXT', 1, Offset + VideoDuration)
...
var CompBlackLayer = FinalComp.layers.addSolid([0, 0, 0], '黑底', CompSize[0], CompSize[1], 1, 1)
// Part 1
var AudioLayer_1 = AddLayer(Part_1, '夏之随想_BGM', 40.5, 0)
AddAudioProperty(AudioLayer_1, 1, 2.3, 38.2, 2)
var BlankLayer_1 = AddLayer(Part_1, '蓝底', 40.5, 0)
var TitleLayer_1 = AddLayer(Part_1, '标题', 4.5, 0)
AddVideoProperty(TitleLayer_1, 1, 0.6, 0, 1)
AddVideoProperty(TitleLayer_1, 1, 0.6, 3.9, 2)
var WarnLayer_1 = AddLayer(Part_1, '注意事项_1', 2.3, 4.7)
AddVideoProperty(WarnLayer_1, 2, 0.3, 4.7, 1)
var WarnLayer_2 = AddLayer(Part_1, '注意事项_2', 2.3, 7)
AddVideoProperty(WarnLayer_2, 2, 0.3, 9, 2)
var BGMLayer = AddLayer(Part_1, '夏之随想', 5.5, 0.5)
AddVideoProperty(BGMLayer, 2, 2, 0, 1)
AddVideoProperty(BGMLayer, 2, 2, 4, 2)
var WorldLayer = AddLayer(Part_1, '哔哩哔哩', 4.5, 9.5)
AddVideoProperty(WorldLayer, 1, 0.3, 9.5, 1)
AddVideoProperty(WorldLayer, 1, 0.3, 13.7, 2)
var StartLayer = AddLayer(Part_1, '集计时间', 4.5, 14.3)
AddVideoProperty(StartLayer, 2, 0.3, 14.3, 1)
AddVideoProperty(StartLayer, 1, 0.3, 18.5, 2)
var OpeningLayer = AddLayer(Part_1, 'Opening', 4.8, 19)
AddVideoProperty(OpeningLayer, 1, 0.5, 19, 1)
AddVideoProperty(OpeningLayer, 1, 0.5, 23.3, 2)
var RuleLayer_1 = AddLayer(Part_1, '规则说明', 4.3, 24)
AddVideoProperty(RuleLayer_1, 2, 0.6, 24, 1)
var RuleLayer_2 = AddLayer(Part_1, '特殊说明', 4.2, 28.3)
var RuleLayer_3 = AddLayer(Part_1, '普通规则', 4, 32.5)
var RuleLayer_4 = AddLayer(Part_1, '新番规则', 4, 36.5)
...
// Part 2
var AudioLayer_2 = AddLayer(Part_2, 'Pickup_BGM', 4, 0)
AddAudioProperty(AudioLayer_2, 1, 1, 0, 1)
AddAudioProperty(AudioLayer_2, 1, 1, 3, 2)
var BlankLayer_2 = AddLayer(Part_2, '蓝底', 4, 0)
var NextLayer_2 = AddLayer(Part_2, 'Pickup切换', 4, 0)
...
// Part 4
var BlankLayer_4 = AddLayer(Part_4, '蓝底', 4, 0)
var NextLayerAudio_4 = AddLayer(Part_4, '主榜切换_BGM', 4, 0)
AddAudioProperty(NextLayerAudio_4, 1, 1, 3, 2)
var NextLayer_4 = AddLayer(Part_4, '主榜切换_1', 4, 0)
...
// Part 6 (tv & bangumi)
var BlankLayer_6 = AddLayer(Part_6, '棕底', 49.1, 0)
var AudioLayer_6 = AddLayer(Part_6, '影视国创_BGM', 49.1, 0 - 30)
...
var FilmLayer = AddLayer(Part_6, '影视切换', 3.73, 0)
AddVideoProperty(FilmLayer, 2, 0.5, 0, 1)
AddVideoProperty(FilmLayer, 2, 0.5, 3.23, 2)
var RankLayer_6_1 = AddLayer(Part_6, '影视副榜_1', 7.6, 4)
AddVideoProperty(RankLayer_6_1, 2, 0.5, 4, 1)
var RankLayer_6_2 = AddLayer(Part_6, '影视副榜_2', 7, 11.6)
var RankLayer_6_3 = AddLayer(Part_6, '影视副榜_3', 7, 18.6)
AddVideoProperty(RankLayer_6_3, 2, 0.6, 25, 2)
var RankLayer_6_cn_sub = AddLayer(Part_6, '国创副榜', 3.6, 26)
AddVideoProperty(RankLayer_6_cn_sub, 2, 0.5, 26, 1)
AddVideoProperty(RankLayer_6_cn_sub, 2, 0.5, 29.1, 2)
var RankLayer_6_bgm_4 = AddLayer(Part_6, '国创副榜_1', 7.5, 30)
AddVideoProperty(RankLayer_6_bgm_4, 2, 0.5, 30, 1)
var RankLayer_6_bgm_5 = AddLayer(Part_6, '国创副榜_2', 8.4, 37.5)
AddVideoProperty(RankLayer_6_bgm_5, 2, 0.5, 44.5, 2)
var RankLayer_6_cn_main = AddLayer(Part_6, '国创切换', 3.7, 45.4)
...
// Part 8
var BlankLayer_8 = AddLayer(Part_8, '蓝底', 4, 0)
var NextLayerAudio_8 = AddLayer(Part_8, '主榜切换_BGM', 4, 0)
AddAudioProperty(NextLayerAudio_8, 1, 1, 3, 2)
var NextLayer_8 = AddLayer(Part_8, '主榜切换_2', 4, 0)
...
// Part 10 (bangumi)
var BlankLayer_10 = AddLayer(Part_10, '棕底', 33, 0)
var AudioLayer_10 = AddLayer(Part_10, '新番副榜_BGM', 33, 0 - 4.3)
...
var BangumiLayer = AddLayer(Part_10, '新番切换', 5, 0)
AddVideoProperty(BangumiLayer, 2, 0.5, 0, 1)
AddVideoProperty(BangumiLayer, 2, 0.5, 4.5, 2)
var BangumiLayer_1 = AddLayer(Part_10, '新番副榜_1', 7, 5.3)
AddVideoProperty(BangumiLayer_1, 2, 0.5, 5.3, 1)
var BangumiLayer_2 = AddLayer(Part_10, '新番副榜_2', 7, 12.3)
var BangumiLayer_3 = AddLayer(Part_10, '新番副榜_3', 7, 19.3)
AddVideoProperty(BangumiLayer_3, 2, 0.5, 25.8, 2)
var BangumiTopLayer = AddLayer(Part_10, '新番主榜', 6.4, 26.6)
...
// Part 12
var BlankLayer_12 = AddLayer(Part_12, '蓝底', 4, 0)
var NextLayerAudio_12 = AddLayer(Part_12, '主榜切换_BGM', 4, 0)
AddAudioProperty(NextLayerAudio_12, 1, 1, 3, 2)
var NextLayer_12 = AddLayer(Part_12, '主榜切换_3', 4, 0)
...
// Part 17
var BlankLayer_17 = AddLayer(Part_17, '棕底', 25, 0)
AddVideoProperty(BlankLayer_17, 1, 0.5, 24.5, 2)
var bilibili_layer = AddLayer(Part_17, '哔哩哔哩_BGM', 25, 0)
bilibili_layer.inPoint = 0
bilibili_layer.outPoint = 25
AddAudioProperty(bilibili_layer, 1, 3, bilibili_layer.outPoint - 3, 2)
var stat_1_layer = AddLayer(Part_17, '统计数据1', 7.33, 0.8)
AddVideoProperty(stat_1_layer, 2, 0.6, 0.8, 1)
var stat_2_layer = AddLayer(Part_17, '统计数据2', 5.27, 8.13)
var stat_3_layer = AddLayer(Part_17, '统计数据3', 5.8, 13.4)
AddVideoProperty(stat_3_layer, 2, 0.6, 18.6, 2)
var over_layer = AddLayer(Part_17, '本周结语', 5.5, 19.5)
AddVideoProperty(over_layer, 2, 0.5, 19.5, 1)
AddVideoProperty(over_layer, 2, 0.5, 24.5, 2)

// Part 18 (30+ to 150 +)
var AudioLayer_18 = AddLayer(Part_18, 'ED_BGM', null, 0)
var EDAudioLength = app.project.items[ResourceID['ED_BGM']].duration
AudioLayer_18.inPoint = 0
AudioLayer_18.outPoint = EDAudioLength
//var BlankLayer_18 = AddLayer(Part_18, '蓝底', 5, 1);
var BlankLayer_18 = AddLayer(Part_18, '棕底', EDAudioLength - 9, 9)
