...
function Normailze {
    ...
    # x264
    $VideoArg = -join @(
        "-y -hide_banner -loglevel error -ss $($Offset) -t $($Length) -i ./ranking/list100/$($FileName).mp4 "
        ...
        "-c:v libx264 -b:v 10M -c:a aac -b:a 320k -r 60 ./ranking/list100/$($RankNum)_$($FileName).mp4"
    )
...
function Main {
    Get-ChildItem './ranking/list100/800.yml' | ForEach-Object {
