...
function Normailze {
    ...
    # Intel Quick Sync
    # $VideoArg = "-y -hide_banner -loglevel error -ss $($Offset) -t $($Length) -init_hw_device qsv=hw -filter_hw_device hw -hwaccel_output_format qsv -i .\ranking\list0\$($FileName).mp4 -vf hwupload=extra_hw_frames=64,format=qsv -af $($Target):print_format=summary:linear=true:$($Source) -b:v 20M -ar 48000 -c:v h264_qsv -c:a aac .\ranking\list1\$($FileName).mp4"
    # CPU
